...
const MeetingForm = ({ meeting, setMeeting }) => {
  ...
  return (
    <>
      ...
      <h3 className="px-4 py-3 mb-4 rounded-md bg-white shadow">
        Schedule a meeting
      </h3>
      <form onSubmit={handleSubmit}>
        ...
        <div className="w-full flex items-center mt-3">
          <a
            className="w-full flex items-center justify-center gap-x-2 py-1.5 px-2 hover:bg-blue-600 rounded-md text-white bg-blue-500 transition-all disabled:opacity-50 mb-3"
            href="https://zoom-clone-harsh.vercel.app"
            target="_blank"
          >
        ...
        <div className="flex flex-col mb-3 relative">
          ...
          {meeting.url ? (
            <h6 className="ml-2 text-gray-500">Meeting url</h6>
          ) : (
            ""
          )}
        </div>
        ...
        {isValidDateTime ? (
          ...
          <h6 className="ml-2 text-red-600">Invalid time</h6>
        )}
        {dateProvided ? (
          ""
        ) : (
          <h6 className="ml-2 text-blue-600">Date required</h6>
        )}
